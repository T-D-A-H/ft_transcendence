      ...
      db.all(streakSql, [userId, userId], (streakErr, rows) => {
        if (!streakErr && rows && rows.length > 0) {
          ...
          for (let i = rows.length - 1; i >= 0; i--) {
            const r = rows[i];
            if (Number(r.winner_id) === userId) stats.currentWinStreak += 1;
            else if (r.winner_id) break;
          }
        }
        requestingUser.send({
          type: "STATS_RESPONSE",
...
function getMatchHistoryRequest(requestingUser, limit) {
  ...
  const historySql = `
    SELECT
      ...
      CASE WHEN m.player2_id IS NULL OR m.is_ai_match = 1 THEN 'local' ELSE 'online' END AS mode,
      CASE
        WHEN m.player2_id IS NULL THEN 'Local'
        WHEN m.is_ai_match = 1 THEN 'AI'
        WHEN m.player1_id = ? THEN COALESCE(u2.display_name, 'Opponent')
        ELSE COALESCE(u1.display_name, 'Opponent')
      END AS opponent
    FROM matches m
