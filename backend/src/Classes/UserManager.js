...
class UserManager {
  ...
  loginUser(userId) {
    ...
    LOGGER(
      ...
      "UserManager",
      "loginUser",
      user.getUsername() + "already logged in.",
    );
    return false;
  }
  ...
  recordMatchResult(match) {
    ...
    if (!winner || !loser || !players || !scores) return;
  ...
  tournamentDisconnect(tournament) {
    LOGGER(
      ...
      "user disconnected from tournament",
    );
  ...
  stopMatch(match) {
    ...
    if (players && players[0] && scores && winner) {
      ...
      const player2Id = match.getIsLocal()
        ? null
        : players[1]
          ? players[1].getId()
          : null;
      const scoreP1 = scores[0];
      const scoreP2 = scores[1];
    ...
    } else match.sendWin(match.getWinner());
  ...
  stopTournament(tournament) {
    ...
    LOGGER(
      ...
      "UserManager",
      "updateTournaments",
      "User: " + tournament.players.get(winner_user).alias + " won the game!",
    );
      ...
      if (tournament.TESTING !== true) {
        ...
        if (tournament.isWaitingAndFull() && tournament.TESTING === false) {
          LOGGER(
            ...
            "USerManager.js",
            "updateTournaments",
            "Tournament is waiting and full",
          );
          tournament.setReady();
          this.createNewTournamentMatches(tournament.getPlayers(), tournament);
        } else if (
          tournament.isRoundFinished() &&
          tournament.TESTING === false
        ) {
  ...
  getAvailableTournaments() {
    ...
    if (tournaments.length === 0) {
      LOGGER(
        ...
        "UserManager",
        "getAvailableTournaments",
        "Called with NO available tournaments",
      );
    ...
    LOGGER(
      ...
      "UserManager",
      "getAvailableTournaments",
      "Called with available tournaments",
    );
    return tournaments;
  }
