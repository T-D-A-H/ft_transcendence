...
type ServerMessage = SearchMatchResponse | CreateMatchResponse | JoinMatchResponse | JoinedMessage;


function sendRequest(userSocket: WebSocket, request: string, payload?: Record<string, any>) {
...
export function joinMatch(userSocket: WebSocket, target_username: string): Promise<void> {
	...
	return new Promise<void>((resolve, reject) => {
		...
		const handler = (event: MessageEvent) => {
			try {
				...
				if (data.type === "JOIN_MATCH_RESPONSE" && data.target === target_username) {
