...
export function registerHandler<T extends ServerMessage>(
  type: T["type"],
  fn: (data: T) => void,
  Constant: boolean,
) {
...
export function oneTimeEvent(
  request: string,
  response: ServerMessage["type"],
  target?: string,
  target2?: string,
): Promise<StatusMsgTarget | null> {
  return new Promise((resolve) => {
    registerHandler(
      response,
      (data) => {
        if (
          data.type === "SEND_INVITE_RESPONSE" ||
...
function onKeyDown(e: KeyboardEvent): void {
  if (matchMode === "single") {
    if (e.key === "w") sendRequest("MOVE", { move: "UP" });
    else if (e.key === "s") sendRequest("MOVE", { move: "DOWN" });
  } else {
    if (e.key === "w") sendRequest("MOVE2", { move: "UP1" });
    else if (e.key === "s") sendRequest("MOVE2", { move: "DOWN1" });
    else if (e.key === "ArrowUp") sendRequest("MOVE2", { move: "UP2" });
    else if (e.key === "ArrowDown") sendRequest("MOVE2", { move: "DOWN2" });
  }
}

function onKeyUp(e: KeyboardEvent): void {
  if (matchMode === "single") {
    if (e.key === "w" || e.key === "s") sendRequest("MOVE", { move: "STOP" });
  } else {
    if (e.key === "w" || e.key === "s") sendRequest("MOVE2", { move: "STOP1" });
